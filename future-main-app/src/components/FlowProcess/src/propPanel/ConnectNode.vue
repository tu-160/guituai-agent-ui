<template>
  ...
  <div class="condition-main overflow-auto p-15px">
    <div class="mb-10px" v-if="formConf.conditions?.length">
      <future-radio v-model:value="formConf.matchLogic" :options="logicOptions" optionType="button" button-style="solid" />
    </div>
      ...
      <div class="condition-item-content">
        ...
        <a-row :gutter="8" v-for="(child, childIndex) in item.groups" :key="index + childIndex" wrap class="mb-10px">
          ...
          <a-col :span="7" class="mt-10px">
            <future-select v-model:value="child.fieldValueType" :options="getConditionTypeOptions" @change="onFieldValueTypeChange(child)" />
          </a-col>
...
<script lang="ts" setup>
  ...
  const props = defineProps(['formConf', 'usedFormItems', 'updateFutureData', 'formFieldsOptions', 'sourceIsStart']);
