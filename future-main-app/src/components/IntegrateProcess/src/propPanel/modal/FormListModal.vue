      ...
      <div class="future-content-wrapper">
        <div class="future-content-wrapper-center">
          <div class="future-content-wrapper-content">
            <BasicTable @register="registerTable" :searchInfo="searchInfo" class="future-sub-table"></BasicTable>
          </div>
        </div>
      </div>
    </a-modal>
...
<script lang="ts" setup>
  ...
  const columns: BasicColumn[] = [
    ...
    { title: '表单类型', dataIndex: 'enableFlow', width: 100, customRender: ({ record }) => (record.enableFlow == '1' ? '流程表单' : '普通表单') },
  ];
  const searchInfo = reactive({
    type: '1',
    isRelease: '1,2',
    enableFlow: props.enableFlow,
    webType: 1,
  });
  ...
  async function openSelectModal() {
    ...
    searchInfo.enableFlow = props.enableFlow;
    visible.value = true;
    nextTick(() => {
      getForm().resetFields();
