...
export const getKbList = async () => {
  ...
  try {
    ...
    const res = await K0001(params);
    const resData = res.data?.kbs;
    kbRt.kbList = resData.map((item: any) => ({
      label: item.name,
      value: item.id,
    }));
    ...
    console.error('Failed to fetch kbList:', error);
    return [];
  }
...
export const menuNodes = [aiChatNode, searchDatasetNode, questionNode, conditionNode, replyNode, rerankerNode];
...
export function isLastNode(nodeModel: any) {
  ...
  return !!(incoming.length > 0 && outcomming.length === 0);
}
