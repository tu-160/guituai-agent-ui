export default {
  ...
  chat_index: {
    ...
    recommendation: '智能体推荐',
    tryEntering: '您可以尝试输入',
    agentDialog: '智能对话',
    enterQuestion: '输入你的问题或需求',
    wonderfulStart: '🌗 凌晨的时光，愿你拥有一个美好的开始！',
    goodMorning: '🌞 早上好！',
    goodafternoon: '🍮 中午好！',
    goodEvening: '🌙 晚上好！',
    more: '发现更多',
  ...
  login: {
    ...
    continue: '继续',
    title: '开始构建您的智能助手',
    description: '探索顶级 RAG 技术。 创建知识库和人工智能来增强您的业务',
    review: '',
    wechatLogin: '欢迎使用微信登录',
    wechatQR: '请使用手机微信扫码登录',
    welcome: '欢迎登录',
  },
  header: {
    knowledgeBase: '知识库',
  ...
  knowledgeDetails: {
    ...
    progressMsg: '进度',
    testingDescription: '最后一步！ 成功后，剩下的就交给 圭图AI 吧。',
    ...
    runningStatus4: '失败',
    pageRanges: '页码范围',
    pageRangesTip: '页码范围：定义需要解析的页面范围。 不包含在这些范围内的页面将被忽略。',
    fromPlaceholder: '从',
    fromMessage: '缺少起始页码',
    toPlaceholder: '到',
    toMessage: '缺少结束页码（不包含）',
    layoutRecognize: '布局识别',
    layoutRecognizeTip: '使用视觉模型进行布局分析，以更好地识别文档结构，找到标题、文本块、图像和表格的位置。 如果没有此功能，则只能获取 PDF 的纯文本。',
    taskPageSize: '任务页面大小',
    ...
    uploadDescription: '支持单次或批量上传。 严禁上传公司数据或其他违禁文件。',
    chunk: '解析块',
    bulk: '批量',
    cancel: '取消',
  ...
  knowledgeConfiguration: {
    ...
    embeddingModelTip: '用于嵌入块的嵌入模型。 一旦知识库有了块，它就无法更改。 如果你想改变它，你需要删除所有的块。',
    ...
    resume: `<p>支持的文件格式为<b>DOCX</b>、<b>PDF</b>、<b>TXT</b>。
      ...
      您只需与<i>'圭图AI'</i>交谈即可列出所有符合资格的候选人。
      </p>
        `,
...
注意您需要指定的条目类型。</p>`,
    ...
    prompt: '提示词',
    promptMessage: '提示词是必填项，LLM提示用于总结。',
...
当您想首先搜索给定的知识库时，请设置比其他知识库更高的 pagerank 得分。`,
  ...
  chat: {
    ...
    systemInitialValue: `你是一个智能助手，请总结知识库的内容来回答问题，请列举知识库中的数据详细回答。当所有知识库内容都与问题无关时，你的回答必须包括“知识库中未找到您要的答案！”这句话。回答需要考虑聊天历史。
        ...
        以上是知识库。`,
    systemMessage: '请输入',
    systemTip: '当LLM回答问题时，你需要LLM遵循的说明，比如角色设计、答案长度和答案语言等。',
    topN: 'Top N',
    ...
    temperatureTip: '该参数控制模型预测的随机性。 较低的温度使模型对其响应更有信心，而较高的温度则使其更具创造性和多样性。',
    topP: 'Top P',
    topPMessage: 'Top P 是必填项',
    topPTip: '该参数也称为“核心采样”，它设置一个阈值来选择较小的单词集进行采样。 它专注于最可能的单词，剔除不太可能的单词。',
    presencePenalty: '存在处罚',
    presencePenaltyMessage: '存在处罚是必填项',
    presencePenaltyTip: '这会通过惩罚对话中已经出现的单词来阻止模型重复相同的信息。',
    frequencyPenalty: '频率惩罚',
    frequencyPenaltyMessage: '频率惩罚是必填项',
    frequencyPenaltyTip: '与存在惩罚类似，这减少了模型频繁重复相同单词的倾向。',
    maxTokens: '最大token数',
    maxTokensMessage: '最大token数是必填项',
    maxTokensTip: '这设置了模型输出的最大长度，以标记（单词或单词片段）的数量来衡量。',
    ...
    answerTitle: '智能回答',
    multiTurn: '多轮对话优化',
    multiTurnTip: '在多轮对话的中，对去知识库查询的问题进行优化。会调用大模型额外消耗token。',
    howUseId: '如何使用聊天ID？',
    description: '助理描述',
    imageSize: '图片大小不得超过500k!',
    editingAgent: '编辑智能体',
    basicInformation: '基本信息',
    presetQuestions: '预设置问题',
    instructions: '智能体指令',
    intelligentParameters: '智能体参数',
    debugging: '调试与预览',
  },
  setting: {
    profile: '概要',
    ...
    baseUrl: 'Base-Url',
    baseUrlTip: '如果您的 API 密钥来自 OpenAI，请忽略它。 任何其他中间提供商都会提供带有 API 密钥的基本 URL。',
    modify: '修改',
    ...
    img2txtModel: '图像转文本模型',
    img2txtModelTip: '所有新创建的知识库都将使用默认的多模块模型。 它可以描述图片或视频。',
    sequence2txtModel: '语音转文本模型',
    sequence2txtModelTip: '所有新创建的知识库都将使用默认的 ASR 模型。 使用此模型将语音翻译为相应的文本。',
    rerankModel: 'Rerank模型',
    ...
    GoogleServiceAccountKeyMessage: '请输入 Google Cloud Service Account Key in base64 format',
  ...
  flow: {
    createAdvanced: '创建高阶智能体',
    flow: '工作流',
    cite: '引用',
    citeTip: '引用历史上下文',
    name: '名称',
    nameMessage: '请输入名称',
    nameExisted: '名称已存在， 请重新输入名称！',
    description: '描述',
    examples: '示例',
    to: '下一步',
    msg: '消息',
    ...
    addMessage: '新增消息',
    loop: '循环上限',
    loopTip: 'loop为当前组件循环次数上限，当循环次数超过loop的值时，说明组件不能完成当前任务，请重新优化agent',
    yes: '是',
    no: '否',
    key: 'key',
    componentId: '组件ID',
    ...
    messageDescription: '此组件用于向用户发送静态信息。您可以准备几条消息，这些消息将被随机选择。',
...
以上就是你需要总结的内容。`,
    ...
    duckDuckGoDescription: '此元件用於從 www.duckduckgo.com 取得搜尋結果。通常，它作為知識庫的補充。 Top N 指定您需要調整的搜尋結果數。',
    channel: '频道',
    channelTip: '针对该组件的输入进行文本搜索或新闻搜索',
    text: '文本',
    news: '新闻',
    messageHistoryWindowSize: '历史消息窗口大小',
    messageHistoryWindowSizeTip: 'LLM 需要查看的对话历史窗口大小。越大越好。但要注意 LLM 的最大内容长度。',
    wikipedia: '维基百科',
    emailTip: '此组件用于从 https://pubmed.ncbi.nlm.nih.gov/ 获取搜索结果。通常，它作为知识库的补充。Top N 指定您需要调整的搜索结果数。电子邮件是必填字段。',
    email: '邮箱',
    pubMed: 'PubMed',
    pubMedDescription:
      '此组件用于从 https://pubmed.ncbi.nlm.nih.gov/ 获取搜索结果。通常，它作为知识库的补充。Top N 指定您需要调整的搜索结果数。电子邮件是必填字段。',
    arXiv: 'ArXiv',
    arXivDescription: '此组件用于从 https://arxiv.org/ 获取搜索结果。通常，它作为知识库的补充。Top N 指定您需要调整的搜索结果数量。',
    sortBy: '排序方式',
    submittedDate: '提交日期',
    lastUpdatedDate: '最后更新日期',
    relevance: '关联',
    google: 'Google',
    googleDescription: '此组件用于从https://www.google.com/获取搜索结果。通常，它作为知识库的补充。Top N 和 SerpApi API 密钥指定您需要调整的搜索结果数量。',
    bing: 'Bing',
    bingDescription: '此组件用于从 https://www.bing.com/ 获取搜索结果。通常，它作为知识库的补充。Top N 和 Bing Subscription-Key 指定您需要调整的搜索结果数量。',
    apiKey: 'API KEY',
    country: '国家和地区',
    language: '语言',
    googleScholar: '谷歌学术',
    ...
    data: '数据',
    deepL: 'DeepL',
    deepLDescription: '该组件用于从 https://www.deepl.com/ 获取翻译。通常，它提供更专业的翻译结果。',
    authKey: '授权键',
    sourceLang: '源语言',
    targetLang: '目标语言',
    gitHub: 'GitHub',
    gitHubDescription: '该组件用于从 https://github.com/ 搜索仓库。Top N 指定需要调整的搜索结果数量。',
    baiduFanyi: '百度翻译',
    baiduFanyiDescription: '该组件用于从 https://fanyi.baidu.com/ 获取翻译。通常，它提供更专业的翻译结果',
    appid: 'App id',
    secretKey: '秘钥',
    domain: '领域',
    transType: '翻译类型',
    ...
    qWeather: '和风天气',
    qWeatherDescription: '该组件用于从 https://www.qweather.com/ 获取天气相关信息。您可以获取天气、指数、空气质量。',
    lang: '语言',
    type: '类型',
    ...
    publish: 'API',
    exeSQL: 'ExeSQL',
    exeSQLDescription: '该组件通过SQL语句从相应的关系数据库中查询结果。支持MySQL，PostgreSQL，MariaDB。',
    dbType: '数据库类型',
    database: '数据库',
    username: '用户名',
    host: '主机',
    ...
    wenCai: '问财',
    queryType: '查询类型',
    wenCaiDescription: '该组件可用于获取广泛金融领域的信息，包括但不限于股票、基金等...',
    ...
    jin10: '金十',
    jin10Description: '该组件可用于从金十开放平台获取金融领域的信息，包括快讯、日历、行情、参考。',
    flashType: '闪光类型',
    filter: '筛选',
    contain: '包含',
    calendarType: '日历类型',
    ...
    concentrator: '集线器',
    concentratorDescription: '该组件可用于连接多个下游组件。它接收来自上游组件的输入并将其传递给每个下游组件。',
    tuShare: 'TuShare',
    tuShareDescription: '该组件可用于从主流金融网站获取金融新闻简报，辅助行业和量化研究。',
    tuShareSrcOptions: {
      sina: '新浪财经',
      wallstreetcn: '华尔街见闻',
      '10jqka': '同花顺',
    ...
    notConnectedTip: '这些节点 {nodes} 不能做为它的下一步。',
  },
  footer: {
